...
usage() {
      cat <<EOF
    Usage: $(basename "${BASH_SOURCE[0]}") [-h] [-v] [-p <aws-profile>] -r <aws-region>  [-P <period>] -c <aws-profile-confinfo> -f <folder>
    [-h]                        : this help message
    [-v]                        : verbose mode
    ...
    -f <folder>               : folder with load tests output
EOF
  exit 1
}
...
end_time=$(tail -1 ../http-output.json | jq | grep time | sed -E 's/"time": "//g' | sed -E 's/"//g' | sed -E 's/ //g' | sed -E 's/,//g' )


echo "=== TIME interval is: $start_time   <====>    $end_time"
echo "=== PERIOD of sampling is: $period"
...
if ( [ -e "../http-output.json" ] ) then
     ...
     cat http_filtered-output.json \
         | jq  -r  '.msg'  \
         | grep -E "HTTP/2.0|X-Amzn-Requestid|X-Amzn-Trace-Id" > monitoring_xray/monitoring_xray_to_search.txt 
     cd monitoring_xray ;
     ...
     startepoch=$(date -d $start_time +"%s")
     endepoch=$(date -d $end_time +"%s")
